...
export class ROIManager {
  ...
  public drawROIs(ctx: CanvasRenderingContext2D, width: number, height: number): void {
    ...
    this.activeCircleROIs.forEach(roi => {
      ...
      // Get the marker state code (D/T/E/R)
      const stateCode = markerStateManager.getStateLetterCode(roi.id);
      ...
      const textMetrics = ctx.measureText(`ID${shortId} ${stateCode}`);
      const textWidth = textMetrics.width;
      ctx.fillStyle = 'rgba(0, 0, 0, 0.5)';
      ctx.fillRect(centerX - textWidth/2 - 2, centerY - 10, textWidth + 4, 14);
      
      // Draw text with state code
      ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
      ctx.fillText(`ID${shortId} ${stateCode}`, centerX, centerY);
    });
  }
