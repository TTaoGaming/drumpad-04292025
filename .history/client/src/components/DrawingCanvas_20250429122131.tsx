...
// Add interface for contour visualization data
...
const DrawingCanvas: React.FC<DrawingCanvasProps> = ({ width, height, enabled, initialPaths = [] }) => {
  ...
  useEffect(() => {
    ...
    if (videoElement) {
      ...
      // Log for debugging
      logger.debug('Canvas', `Video display size: ${videoDisplayWidth}x${videoDisplayHeight}`);
      logger.debug('Canvas', `Canvas size: ${canvasSize.width}x${canvasSize.height}`);
    }
    ...
    const pinchStateListener = addListener(
      ...
      (data) => {
        ...
        if (data.section === 'gestures' && data.setting === 'pinchState' && settings.enabled) {
          ...
          logger.debug('Canvas', `Received pinch event - isPinching: ${isPinching}, distance: ${distance.toFixed(3)}`);
          
          // Debug which finger is being used (always index finger now)
          if (fingerId !== undefined) {
            logger.debug('Canvas', `Active finger: ${activeFinger} (ID: ${fingerId})`);
          }
          ...
          if (data.value.position) {
            ...
            logger.debug('Canvas', `Pinch position: (${position.x}, ${position.y})`);
      ...
      (data) => {
        if (data.section === 'tracking' && data.setting === 'thumbPosition' && settings.enabled) {
          ...
          if (isDrawing && position && currentPath && currentPath.points.length > 0) {
            ...
            // Log for debugging
            logger.debug('Canvas', `Continuing drawing with thumb at (${Math.round(position.x)}, ${Math.round(position.y)})`);
          }
        }
      }
    );
    ...
    const settingsListener = addListener(
      ...
      (data) => {
        ...
        if (data.section === 'drawing' && data.setting === 'longPressDelay' && typeof data.value === 'number') {
          logger.debug('Canvas', 'Updating long press delay to', data.value, 'ms');
    ...
    const roiUpdatedListener = addListener(EventType.ROI_UPDATED, (data) => {
      ...
      if (data && data.id && data.center) {
        // Update the corresponding path with the new center position
        setPaths(prevPaths => prevPaths.map(path => {
          if (path.id === data.id && path.isROI && path.isComplete) {
            // Get the current center of this path
            const currentCenter = calculateCenter(path.points);
            
            // Calculate the new center position in pixels
            const newCenterX = data.center.x * canvasSize.width;
            const newCenterY = data.center.y * canvasSize.height;
            
            // Move each point by the difference between old and new centers
            const updatedPoints = path.points.map(point => {
              // Calculate vector from old center to point
              const dx = point.x - currentCenter.x;
              const dy = point.y - currentCenter.y;
              
              // Apply this vector to the new center
              return {
                x: newCenterX + dx,
                y: newCenterY + dy
              };
            ...
            // Log movement occasionally (only 5% of updates to avoid console spam)
            if (Math.random() < 0.05) {
              logger.debug('Canvas', `Moving marker #${path.colorIndex || 1} from (${Math.round(currentCenter.x)}, ${Math.round(currentCenter.y)}) to (${Math.round(newCenterX)}, ${Math.round(newCenterY)})`);
            }
            
            // Return updated path with new points
            return { ...path, points: updatedPoints };
          }
          return path;
        }));
        
        // Check for contour visualization data
        if (data.contourTracking && data.contourTracking.visualizationData) {
          setContourVisualizations(prev => ({
            ...prev,
            [data.id]: {
              roiId: data.id,
          ...
          if (Math.random() < 0.05) {
            logger.debug('Canvas', `Updated contour visualization for ROI ${data.id}, occluded: ${data.contourTracking.isOccluded}`);
            if (data._roiRegion) {
              logger.debug('Canvas', `ROI region: (${data._roiRegion.x}, ${data._roiRegion.y}) size: ${data._roiRegion.size}`);
            }
          }
        }
      }
            ...
            if (tempCtx) {
              ...
              if (viz.roiRegion) {
                ...
                if (Math.random() < 0.005) {
                  logger.debug('Canvas', `Drawing aligned contour viz at (${Math.round(vizX)}, ${Math.round(vizY)}) with diameter ${Math.round(vizSize)}`);
                }
              } else {
  ...
  }, [paths, currentPath, canvasSize, settings, contourVisualizations]);
  ...
  const handlePinchStateChange = (isPinching: boolean, position: { x: number, y: number }, fingerId?: number) => {
    logger.debug('Canvas', `Pinch state change: isPinching=${isPinching}, position=(${Math.round(position.x)}, ${Math.round(position.y)}), isDrawing=${isDrawing}`);
    ...
    if (isPinching && !wasPinching && !isDrawing) {
      ...
      logger.debug('Canvas', 'New pinch detected, starting long press timer with delay of', longPressDelay, 'ms');
      ...
      longPressTimerRef.current = setTimeout(() => {
        ...
        if (isPinchingRef.current && pinchPositionRef.current && isLongPressActiveRef.current) {
          logger.debug('Canvas', 'Long press timer completed, starting to draw');
          ...
          logger.debug('Canvas', 'Long press timer completed, but pinch already released');
          isLongPressActiveRef.current = false;
        }
    ...
    // CASE 2: Continue drawing when already drawing and still pinching
      ...
      logger.debug('Canvas', 'Continuing drawing with thumb pinch at:', position);
      addPointToPath(position.x, position.y);
    }
    // CASE 3: Pinch released
    else if (!isPinching) {
      logger.debug('Canvas', 'Pinch released, was drawing:', isDrawing);
      
      // Clear any pending long press timer
      if (longPressTimerRef.current) {
        logger.debug('Canvas', 'Canceling long press timer due to pinch release');
        clearTimeout(longPressTimerRef.current);
        longPressTimerRef.current = null;
        isLongPressActiveRef.current = false;
      }
      ...
      if (isDrawing) {
        logger.debug('Canvas', 'Stopping drawing with points:', currentPath ? currentPath.points.length : 0);
        stopDrawing();
      }
    }
  };
  ...
  const addPointToPath = (x: number, y: number) => {
    ...
    logger.trace('Canvas', `Adding point: (${Math.round(x)}, ${Math.round(y)})`);
    ...
    setCurrentPath(prev => {
      ...
      if (distance < 2) {
        ...
        logger.trace('Canvas', `  Point too close to last point (${Math.round(distance)}px), skipping`);
        return prev;
      }
      
      // Log addition of the point
      logger.trace('Canvas', `  Added point at distance ${Math.round(distance)}px from last point`);
  ...
  const stopDrawing = () => {
    ...
    if (currentPath.isROI) {
      ...
      if (currentPath.points.length < 3) {
        ...
        logger.debug('Canvas', `Created default circle with radius ${radius}px at (${center.x}, ${center.y})`);
      } 
      else {
        // We have enough points for a proper shape
        logger.debug('Canvas', `Processing ${currentPath.points.length} points for ROI creation`);
        ...
        logger.debug('Canvas', `Measured drawing diameter: ${diameter}px (radius: ${radius}px)`);
        logger.debug('Canvas', `Creating circle with center (${Math.round(center.x)}, ${Math.round(center.y)}) and radius ${Math.round(radius)}px`);
  ...
  useEffect(() => {
    if (initialPaths.length > 0) {
      logger.debug('Canvas', 'Initializing paths with:', initialPaths.length, 'paths');
    ...
    const updateCanvasSize = () => {
      ...
      if (videoElement) {
        ...
        if (videoDisplayWidth !== canvasSize.width || videoDisplayHeight !== canvasSize.height) {
          logger.debug('Canvas', `Canvas size updated to match video: ${videoDisplayWidth}x${videoDisplayHeight}`);
        ...
        logger.debug('Canvas', `Video display size: ${width}x${height}`);
        logger.debug('Canvas', `Canvas size: ${width}x${height}`);
