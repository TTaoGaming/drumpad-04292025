...
function App() {
  ...
  useEffect(() => {
    ...
    // Listen for marker state changes
    const markerStateListener = addListener(EventType.MARKER_STATE_CHANGED, (data) => {
      // Log state changes
      const stateChangeMessage = `Marker ${data.markerId} changed from ${data.prevState} to ${data.newState}`;
      addLog(stateChangeMessage, 'info');
      
      // Show notification for transitions to ENGAGED and RELEASE states
      if (data.newState === 'ENGAGED') {
        addNotification(`Marker ${data.markerId} is now ENGAGED`, 'success');
      } else if (data.newState === 'RELEASE') {
        addNotification(`Marker ${data.markerId} RELEASED`, 'info');
      }
    });
    ...
    if (timeSinceLastFrame >= frameInterval) {
      ...
      if (now - performanceCounterRef.current.lastFpsUpdate > 1000) {
        ...
        if (++frameLogCountRef.current % LOG_EVERY_N_FRAMES === 0) {
          logger.debug('App', `Actual FPS: ${fps.toFixed(1)}`);
        }
  ...
  const addLog = (message: string, type: 'info' | 'error' | 'success' | 'warning' = 'info') => {
    ...
    // Use our centralized logger instead of console.log
    switch (type) {
      case 'error':
